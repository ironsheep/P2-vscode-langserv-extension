...
| `spin2.fDirSource` | Directory containing the active `.spin2` source file, used as the working directory for compile and download tasks | **Runtime set** automatically when the active editor changes — ensures tasks run in the source file's directory, not the workspace root
