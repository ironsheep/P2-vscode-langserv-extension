...
export class Spin1DocumentSemanticParser {
  ...
  private _parseText(text: string): IParsedToken[] {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      let trimmedNonCommentLine: string = bHaveLineToProcess
        ? this.parseUtils.getRemainderWOutTrailingTicComment(0, lineWOutInlineComments).trimStart()
        : '';
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (currState == eParseState.inMultiLineDocComment) {
        ...
        //this._logMessage(`    hunt for '}}' Ln#${lineNbr} trimmedNonCommentLine=[${trimmedNonCommentLine}]`);
        ...
        // in multi-line non-doc-comment, hunt for end '}' to exit
        // ALLOW {...} on same line without closing!
        //this._logMessage(`    hunt for '}' Ln#${lineNbr} lineWOutInlineComments=[${lineWOutInlineComments}]`);
        const closingOffset: number = lineWOutInlineComments.indexOf('}');
        if (closingOffset != -1) {
          // have close, comment ended
  ...
  private _getDAT_Declaration(startingOffset: number, lineNbr: number, line: string): void {
    ...
    const isDatDeclLine: boolean = lineParts.length >= 1 && lineParts[0].toUpperCase() == 'DAT' ? true : false;
    if ((isDatDeclLine && lineParts.length >= 2) || (!isDatDeclLine && lineParts.length > 0)) {
        ...
        if (newName.length > 0) {
          ...
          if (!this.parseUtils.isSpinReservedWord(newName) && !this.parseUtils.isBuiltinReservedWord(newName) && !notOKSpin2Word) {
            ...
            if (!isDataDeclarationLine) {
              if (newName.startsWith('.')) {
                ...
                this.semanticFindings.pushDiagnosticMessage(
                  ...
                  eSeverity.Error,
                  `P2 pasm local name [${newName}] not supported in P1 pasm`
                );
            ...
            const fileName: string | undefined =
              isNamedDataDeclarationLine && bIsFileLine && lineParts.length > maxParts ? lineParts[maxParts] : undefined;
            ...
            this.semanticFindings.pushDiagnosticMessage(
              ...
              `Possible use of P2 Spin reserved word [${newName}]`
            );
          }
        }
      }
      ...
      const valueDeclNonCommentStr: string =
        isDataDeclarationLine && dataTypeOffset != -1 ? dataDeclNonCommentStr.substring(dataTypeOffset + dataType.length).trim() : '';
              ...
              if (haveObjectRef) {
                ...
                if (objName.charAt(0).match(/[a-zA-Z_]/) && objRef.charAt(0).match(/[a-zA-Z_]/)) {
                  ...
                  if (bISMethod) {
                    this.semanticFindings.setGlobalToken(
                      objRef,
                      new RememberedToken('method', lineNbr - 1, nameOffset, []),
                      this._declarationComment(),
                      objName
                    );
                  } else {
          ...
          if (!isDataDeclarationLine) {
            if (labelName.startsWith('.')) {
              ...
              this.semanticFindings.pushDiagnosticMessage(
                lineNbr - 1,
                offset,
                offset + labelName.length,
                eSeverity.Error,
                `P2 pasm local name [${labelName}] not supported in P1 pasm`
              );
    ...
    if (fileName) {
      ...
      if (hasPathSep) {
        this.semanticFindings.pushDiagnosticMessage(
          ...
          eSeverity.Error,
          `P1 spin Invalid filename character "/" in [${filenameNoQuotes}]`
        );
    ...
    if (fileName) {
      ...
      if (hasPathSep) {
        ...
        this.semanticFindings.pushDiagnosticMessage(
          ...
          nameOffset + filenameNoQuotes.length,
          eSeverity.Error,
          `Missing P1 Object file [${displayName}]`
        );
      }
    }
  }
  ...
  private _getVAR_Declaration(startingOffset: number, lineNbr: number, line: string): void {
    ...
    if (remainingNonCommentLineStr.length > 0) {
      ...
      const lineParts: string[] = this.parseUtils.getNonWhiteDataInitLineParts(remainingNonCommentLineStr);
    ...
    if (nonCommentConstantLine.length > 0) {
      ...
      if (this.configuration.highlightFlexspinDirectives) {
        ...
        this.semanticFindings.pushDiagnosticMessage(
          ...
          eSeverity.Error,
          `P1 Spin - PreProcessor Directive [${lineParts[0]}] not supported!`
        );
      }
    }
  ...
  private _reportCON_DeclarationLine(lineIdx: number, startingOffset: number, line: string): IParsedToken[] {
    ...
    this._logCON(
      '- reportConstant haveEnum=(' +
        haveEnumDeclaration +
        '), containsMulti=(' +
        containsMultiAssignments +
      ...
      if (this.parseUtils.isStorageType(lineParts[0]) || lineParts[0].toUpperCase() == 'FILE' || lineParts[0].toUpperCase() == 'ORG') {
        ...
        if (referenceDetails !== undefined) {
          ...
          this.semanticFindings.pushDiagnosticMessage(
            ...
            eSeverity.Error,
            `P1 Spin DAT missing declaration [${newName}]`
          );
        }
            ...
            if (referenceDetails !== undefined) {
              ...
              if (
                ...
                this.semanticFindings.pushDiagnosticMessage(
                  ...
                  nameOffset + namePart.length,
                  eSeverity.Error,
                  `P1 Spin DAT missing declaration [${namePart}]`
                );
              }
            }
          }
  ...
  private _reportDAT_PAsmCode(lineIdx: number, startingOffset: number, line: string): IParsedToken[] {
    ...
    if (inLinePAsmRHSStr.length > 0) {
      ...
      const isDataDeclarationLine: boolean =
        (lineParts.length > 1 && haveLabel && this.parseUtils.isDatStorageType(lineParts[1])) ||
        (lineParts.length > 0 && !haveLabel && this.parseUtils.isDatStorageType(lineParts[0]));
      ...
      if (haveLabel) {
        ...
        if (referenceDetails !== undefined) {
          ...
          this.semanticFindings.pushDiagnosticMessage(
            ...
            eSeverity.Error,
            `Not a legal P1 pasm label [${labelName}]`
          );
          haveLabel = true;
        }
      }
      ...
      if (!isDataDeclarationLine) {
        ...
        if (lineParts.length > 1) {
          ...
          const nameOffset: number = line.indexOf(namePart, currentOffset);
          ...
          this.semanticFindings.pushDiagnosticMessage(
            ...
            nameOffset + namePart.length,
            eSeverity.Error,
            `P2 pasm local name [${namePart}] not supported in P1 pasm`
          );
        }
      } else {
                  ...
                  if (
                    ...
                    if (this.parseUtils.isP2SpinMethod(namedIndexPart)) {
                      ...
                      this.semanticFindings.pushDiagnosticMessage(
                        ...
                        nameOffset + namedIndexPart.length,
                        eSeverity.Error,
                        `P1 Spin A missing declaration [${namedIndexPart}]`
                      );
                    }
                  }
                }
    ...
    if (remainingLength > 0) {
      ...
      if (nonCommentSpinLine.toLowerCase().includes('else if')) {
        ...
        this.semanticFindings.pushDiagnosticMessage(
          ...
          eSeverity.Error,
          'Illegal "else if" form for P1 Spin'
        );
      }
      // locate key indicators of line style
                  ...
                  if (referenceDetails !== undefined) {
                    ...
                    if (
                      ...
                      this.semanticFindings.pushDiagnosticMessage(
                        ...
                        eSeverity.Error,
                        `P1 Spin B missing declaration [${variableNamePart}]`
                      );
                    }
                  }
                }
              }
              currentOffset = nameOffset + variableNamePart.length;
            }
          } else {
        ...
        if (possibleName.charAt(0).match(/[a-zA-Z_]/)) {
          ...
          if (possibleName.toLowerCase() === 'org' || possibleName.toLowerCase() === 'org') {
            ...
            this.semanticFindings.pushDiagnosticMessage(
              lineIdx,
              nameOffset,
              nameOffset + possibleName.length,
              eSeverity.Error,
              `In-line Pasm not allowed in P1 Spin [${possibleName}]`
            );
            return tokenSet;
          }
            ...
            if (this.parseUtils.isStorageType(namePart)) {
              ...
              if (
                ...
                this.semanticFindings.pushDiagnosticMessage(
                  ...
                  nameOffset + namePart.length,
                  eSeverity.Information,
                  `Possible use of P2 Spin reserved word [${namePart}]`
                );
              } else {
                this.semanticFindings.pushDiagnosticMessage(
                  lineIdx,
                  ...
                  nameOffset + namePart.length,
                  eSeverity.Error,
                  `P1 Spin D missing declaration [${namePart}]`
                );
              }
            } else {
              ...
              if (this.semanticFindings.isGlobalToken(nameReference)) {
                ...
                // we don't have name registered so just mark it
                  ...
                  this.semanticFindings.pushDiagnosticMessage(
                    ...
                    eSeverity.Error,
                    `P1 Spin E missing declaration [${nameReference}]`
                  );
                }
              }
            }
          ...
          if (newName.charAt(0).match(/[a-zA-Z_]/) && newName.indexOf(']') == -1) {
            ...
            if (referenceDetails !== undefined) {
              ...
              this.semanticFindings.pushDiagnosticMessage(
                ...
                nameOffset + newName.length,
                eSeverity.Error,
                `P1 Spin F missing declaration [${newName}]`
              );
            }
                  ...
                  if (referenceDetails !== undefined) {
                    ...
                    if (!this.parseUtils.isSpinReservedWord(namePart) && !this.parseUtils.isBuiltinReservedWord(namePart)) {
                      ...
                      this.semanticFindings.pushDiagnosticMessage(
                        ...
                        nameOffset + namePart.length,
                        eSeverity.Error,
                        `P1 Spin G missing declaration [${namePart}]`
                      );
                    }
                  }
                }
      ...
      if (this.semanticFindings.isNameSpace(objInstanceName)) {
        ...
        if (referenceDetails !== undefined) {
          ...
          if (possibleNameSet.length > 1) {
            ...
            const isMethod: boolean = !isObjectConstantRef;
