...
export class Spin2DocumentSemanticParser {
  ...
  private _parseText(text: string): IParsedToken[] {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // strip comments before checking for debug() to prevent false matches from debug() in comments
      const lineWithoutInlineComments: string = !bHaveEmptyLine ? this.parseUtils.getLineWithoutInlineComments(line) : '';
      const lineWithoutComments: string = !bHaveEmptyLine
        ? this.parseUtils.getRemainderWOutTrailingComment(0, lineWithoutInlineComments)
        : '';
      const isDebugLine: boolean = !bHaveEmptyLine && !trimmedLine.startsWith("'") ? haveDebugLine(lineWithoutComments) : false;
      ...
      if (isDebugLine) {
        nonCommentLine = this._getDebugNonCommentLine(0, lineWithoutInlineComments);
      } else {
        nonCommentLine = lineWithoutComments;
      }
        ...
        if (bHaveLineToProcess) {
          ...
          if (lineParts.length > 0 && (lineParts[0] === 'ENDASM' || lineParts[0] === 'END')) {
            ...
            if (lineParts[0] === 'ENDASM' && !this.configuration.highlightFlexspinDirectives) {
              ...
              this._createAndRecordToken(tokenSet, line, lineNbr - 1, nameOffset, lineParts[0].length, 'directive', []);
            }
            // and ignore rest of this line
          } else {
            // process pasm code
  ...
  private _getDAT_Declaration(startingOffset: number, lineNbr: number, line: string): void {
    ...
    if (lineParts.length >= minDecodeCount) {
      ...
      const isExternalObjectType: boolean = dataType !== undefined && dataType.includes('.');
      if (dataType !== undefined && !this.parseUtils.isDatNFileStorageType(dataType) && !this.isStorageType(dataType) && !isExternalObjectType) {
      ...
      const haveStorageType: boolean = dataType ? (this.isStorageType(dataType) || isExternalObjectType) : false;
      ...
      if (
        ...
        if (isExternalObjectType && dataType) {
          this.semanticFindings.recordStructureInstance(dataType, newName); // DAT
        }
      }
    }
  }
  ...
  private _getDAT_PAsmDeclaration(startingOffset: number, lineNbr: number, line: string): void {
    ...
    if (datPAsmRHSStr.length > 0) {
      ...
      const typeField: string = lineParts.length > 1 ? lineParts[1] : '';
      const isExternalObjectType: boolean = typeField.includes('.');
      const isDataDeclarationLine: boolean =
        lineParts.length > 1 && haveLabel && (this.parseUtils.isDatStorageType(typeField) || this.isStorageType(typeField) || isExternalObjectType)
          ? true
          : false;
      if (haveLabel) {
        ...
        if (
          ...
          if (labelName.toUpperCase() === 'DITTO') {
            ...
            if (isExternalObjectType && typeField.length > 0) {
              this.semanticFindings.recordStructureInstance(typeField, labelName); // DAT PAsm
            }
          }
        }
      }
    ...
    if (remainingNonCommentLineStr.length >= 5) {
      ...
      if (remainingNonCommentLineStr.includes(':') && remainingNonCommentLineStr.includes('"') && !remainingNonCommentLineStr.endsWith(':')) {
        ...
        if (lineParts.length < 2) {
          ...
          // record document link for OBJ filename (inside quotes)
          const objOpenQuoteOffset: number = line.indexOf('"', currentOffset);
          if (objOpenQuoteOffset !== -1) {
            const objCloseQuoteOffset: number = line.indexOf('"', objOpenQuoteOffset + 1);
            if (objCloseQuoteOffset !== -1) {
              this.semanticFindings.recordDocumentLink({
                line: lineNbr - 1,
                startCharacter: objOpenQuoteOffset + 1,
                endCharacter: objCloseQuoteOffset,
                targetFilename: filenamePart,
                isInclude: false
              });
            }
          }
  ...
  private _getVAR_Declaration(startingOffset: number, lineNbr: number, line: string): void {
    ...
    if (remainingNonCommentLineStr.length > 0) {
      ...
      for (let index = 0; index < declStatements.length; index++) {
        ...
        const isExternalObjectType: boolean = typeName.includes('.');
        if (typeName.length > 0) {
          if (!this.isStorageType(typeName) && !this.parseUtils.isAlignType(typeName) && !isExternalObjectType && !this.semanticFindings.isStructure(typeName)) {
        ...
        const isStructureType: boolean = typeName.length > 0 ? (this.semanticFindings.isStructure(typeName) || isExternalObjectType) : false;
  ...
  private _reportPreProcessorLine(lineIdx: number, startingOffset: number, line: string, isPNut: boolean): IParsedToken[] {
    const tokenSet: IParsedToken[] = [];
    ...
    if (nonCommentConstantLine.length > 0) {
      // get line parts - we only care about first one
      const lineParts: string[] = nonCommentConstantLine.split(/[ \t=]/).filter(Boolean);
      const logPrefix = isPNut ? 'pnut' : 'flexspin';
      this._logPreProc(`  - Ln#${lineNbr} ${logPrefix} reportPreProc lineParts=[${lineParts}]`);
      const directive: string = lineParts[0];
      const symbolName: string | undefined = lineParts.length > 1 ? lineParts[1] : undefined;

      // Check if this is a valid preprocessor directive
      const isValidDirective = isPNut
        ? this.parseUtils.isPNutPreprocessorDirective(directive)
        : this.parseUtils.isFlexspinPreprocessorDirective(directive);

      // For FlexSpin, check if highlighting is enabled
      if (!isPNut && !this.configuration.highlightFlexspinDirectives) {
        // FlexSpin support not enabled - report error
        this._recordTokenWithDiagnostic(
          tokenSet,
          line,
          lineIdx,
      ...
      if (isValidDirective) {
        ...
        const directiveOffset: number = isPNut ? line.indexOf(directive, currentOffset) : 0;
        this._createAndRecordToken(tokenSet, line, lineIdx, directiveOffset, directive.length, 'keyword', ['control', 'directive']);
        ...
        if (lineHasSymbol && symbolName !== undefined) {
          ...
          const referenceDetails = this._getGlobalTokenIfExists(symbolName, 'PreProc');
          if (referenceDetails !== undefined) {
            ...
            this._createAndRecordToken(tokenSet, line, lineIdx, nameOffset, symbolName.length, referenceDetails.type, updatedModificationSet);
          } else if (this.semanticFindings.isPreProcSymbolDefined(symbolName)) {
            this._createAndRecordToken(tokenSet, line, lineIdx, nameOffset, symbolName.length, 'variable', ['readonly']);
          } else if (!isPNut && this.parseUtils.isFlexspinReservedWord(symbolName)) {
            // FlexSpin-specific: record a constant reference
            this._createAndRecordToken(tokenSet, line, lineIdx, nameOffset, symbolName.length, 'variable', ['readonly']);
          } else {
            // record an unknown name
            this._createAndRecordToken(tokenSet, line, lineIdx, nameOffset, symbolName.length, 'variable', ['disabled']);
          }
        }
          ...
          if (lineHasFilename) {
            ...
            if (openQuoteOffset !== -1) {
              ...
              if (closeQuoteOffset !== -1) {
                ...
                // record an filename
                this._createAndRecordToken(tokenSet, line, lineIdx, openQuoteOffset, symbolLength, 'filename', []);
                this._logPreProc(`  -- filename=[${line.substring(openQuoteOffset, closeQuoteOffset + 1)}]`);
                // record document link for #include filename (inside quotes)
                const includeFilename: string = line.substring(openQuoteOffset + 1, closeQuoteOffset);
                this.semanticFindings.recordDocumentLink({
                  line: lineIdx,
                  startCharacter: openQuoteOffset + 1,
                  endCharacter: closeQuoteOffset,
                  targetFilename: includeFilename,
                  isInclude: true
                });
              }
            }
  ...
  private _reportPNutPreProcessorLine(lineIdx: number, startingOffset: number, line: string): IParsedToken[] {
    return this._reportPreProcessorLine(lineIdx, startingOffset, line, true);
  }

  private _reportFlexspinPreProcessorLine(lineIdx: number, startingOffset: number, line: string): IParsedToken[] {
    return this._reportPreProcessorLine(lineIdx, startingOffset, line, false);
  }
    ...
    if (lineParts.length > 1) {
      ...
      if (this.isStorageType(lineParts[0]) || lineParts[0].toUpperCase() === 'FILE' || lineParts[0].toUpperCase() === 'ORG') {
        ...
        if (referenceDetails !== undefined) {
          ...
          this._createAndRecordToken(tokenSet, line, lineIdx, nameOffset, newName.length, referenceDetails.type, modifiersWDecl, newName);
          if (bIsAlsoDebugLine) {
          ...
          if (this.parseUtils.requestedSpinVersion(50)) {
            ...
            this._createAndRecordToken(tokenSet, line, lineIdx, nameOffset, newName.length, 'directive', []);
            if (lineParts[1].toUpperCase() === 'END') {
              ...
              this._createAndRecordToken(tokenSet, line, lineIdx, nameOffset, lineParts[1].length, 'directive', []);
            }
          } else {
      ...
      if (!this.parseUtils.isAlignType(newName)) {
        ...
        if (referenceDetails !== undefined) {
          ...
          const modifiersWDecl: string[] = referenceDetails.modifiersWith('declaration');
          this._createAndRecordToken(tokenSet, line, lineIdx, currentOffset, newName.length, referenceDetails.type, modifiersWDecl, newName);
        } else if (
          this.parseUtils.isP1AsmInstruction(newName) ||
          this.parseUtils.isBadP1AsmEffectOrConditional(newName) ||
          this.parseUtils.isP1AsmVariable(newName)
      ...
      if (lineParts.length > 1) {
        ...
        for (let index = 0; index < lineParts.length; index++) {
          ...
          if (paramIsNumber) {
            ...
            this._createAndRecordToken(tokenSet, line, lineIdx, nameOffset, possibleName.length, 'number', []);
            ...
            if (this._isPossibleObjectReference(possibleName)) {
              ...
              this._createAndRecordToken(tokenSet, line, lineIdx, nameOffset, possibleName.length, 'storageType', []);
              currentOffset = nameOffset + possibleName.length;
              ...
              this._createAndRecordToken(tokenSet, line, lineIdx, nameOffset, possibleName.length, 'operator', ['builtin']);
              currentOffset = nameOffset + possibleName.length;
              continue;
            }
            ...
            if (referenceDetails !== undefined) {
              this._createAndRecordToken(tokenSet, line, lineIdx, nameOffset, namePartLength, referenceDetails.type, referenceDetails.modifiers, namePart);
            } else {
              if (
                !this.parseUtils.isP2AsmReservedWord(namePart) &&
    ...
    if (inLinePAsmRHSStr.length > 0) {
      ...
      if (haveLabel) {
        ...
        if (referenceDetails !== undefined) {
          ...
          const modifiersWDecl: string[] = referenceDetails.modifiersWith('declaration');
          this._createAndRecordToken(tokenSet, line, lineIdx, nameOffset, labelName.length, referenceDetails.type, modifiersWDecl, labelName);
          haveLabel = true;
          ...
          if (!this.parseUtils.requestedSpinVersion(50)) {
            ...
            if (lineParts.length > 1) {
              ...
              if (argument.toUpperCase() === 'END') {
                this._createAndRecordToken(tokenSet, line, lineIdx, nameOffset, labelName.length, 'variable', ['illegalUse']);
              }
            }
            haveLabel = true;
          }
          ...
          if (lineParts.length > minNonLabelParts) {
            ...
            if (likelyInstructionName.toUpperCase() === 'DITTO') {
              ...
              if (this.parseUtils.requestedSpinVersion(50)) {
                ...
                this._createAndRecordToken(tokenSet, line, lineIdx, nameOffset, likelyInstructionName.length, 'directive', []);
              } else {
                // if NOT version 50  DITTO and DITTO END are illegal
                this._recordTokenWithDiagnostic(
                  tokenSet,
            ...
            for (let index = minNonLabelParts; index < lineParts.length; index++) {
              ...
              if (argumentName.toLowerCase() === 'end') {
                ...
                if (this.parseUtils.requestedSpinVersion(50) && bIsDittoLine) {
                  ...
                  // color our 'ditto end' token
                  this._createAndRecordToken(tokenSet, line, lineIdx, nameOffset, argumentName.length, 'directive', []);
                } else {
                  // if NOT version 50  DITTO and DITTO END are illegal
                  this._createAndRecordToken(tokenSet, line, lineIdx, nameOffset, lineParts[1].length, 'variable', ['illegalUse']);
                }
              ...
              if (argumentName.charAt(0).match(/[a-zA-Z_.:]/)) {
                ...
                if (referenceDetails !== undefined) {
                  ...
                  if (
                    ...
                    this._createAndRecordToken(tokenSet, line, lineIdx, nameOffset, namePart.length, 'variable', ['illegalUse']);
                    /*
              ...
              if (!foundObjectRef) {
                ...
                if (
                  ...
                  this._recordToken(tokenSet, multiLineSet.lineAt(symbolPosition.line), {
                    ...
                    ptTokenType: 'storageType',
                    ptTokenModifiers: []
                  }, !this.parseUtils.isStorageType(typeName) ? typeName : undefined);
                  structureType = typeName;
            ...
            if (!foundObjectRef) {
              ...
              if (
                ...
                this._recordToken(tokenSet, multiLineSet.lineAt(symbolPosition.line), {
                  ...
                  ptTokenType: 'storageType',
                  ptTokenModifiers: []
                }, !this.parseUtils.isStorageType(storageType) ? storageType : undefined);
  ...
  private _reportSPIN_PAsmCode(lineIdx: number, startingOffset: number, line: string): IParsedToken[] {
    ...
    if (inLinePAsmRHSStr.length > 0) {
      ...
      if (haveLabel) {
        ...
        this._createAndRecordToken(tokenSet, line, lineIdx, nameOffset, labelName.length, labelType, labelModifiers);
        haveLabel = true;
      }
      if (bIsAlsoDebugLine) {
        ...
        if (lineParts.length > 1) {
          ...
          if (possibleDirective.toUpperCase() === 'FILE') {
            ...
            if (this.parseUtils.requestedSpinVersion(50)) {
              ...
              this._createAndRecordToken(tokenSet, line, lineIdx, nameOffset, possibleDirective.length, 'directive', []);
              if (lineParts[1].toUpperCase() === 'END') {
                // color our 'ditto end' token
        ...
        for (let index = 0; index < varStatements.length; index++) {
          ...
          if (newType.length > 0) {
            ...
            if (!this.parseUtils.isAlignType(newType)) {
              // record struct type names as references (but not built-in BYTE/WORD/LONG)
              const typeRefName = this.semanticFindings.isStructure(newType) ? newType : undefined;
              this._createAndRecordToken(tokenSet, line, lineIdx, nameOffset, newType.length, 'storageType', [], typeRefName);
            }
          ...
          if (this.parseUtils.isValidSpinSymbolName(newName)) {
            ...
            this._createAndRecordToken(tokenSet, line, lineIdx, nameOffset, newName.length, 'variable', ['declaration', 'instance'], newName);
        ...
        if (!bIsStructureInstance && this.semanticFindings.isNameSpace(objInstanceName)) {
          ...
          if (referenceDetails !== undefined) {
            ...
            if (possibleNameSet.length > 1) {
              ...
              if (referenceDetails && !isStructure && !onlyStructRefs && (isMethod || (!isMethod && !objectRefContainsIndex))) {
                ...
                this._createAndRecordToken(tokenSet, line, lineIdx, referenceOffset, refPart.length, referenceDetails.type, tokenModifiers, refPart);
              } else if (isStructure) {
                ...
                this._createAndRecordToken(tokenSet, line, lineIdx, structureOffset, structurePart.length, 'storageType', ['readonly'], structurePart);
                ...
                this._createAndRecordToken(tokenSet, line, lineIdx, referenceOffset, refPart.length, 'variable', ['illegalUse'], refPart);
          ...
          if (!bIsStructureInstance && possibleNameSet.length > 1) {
            ...
            // NO if either side is not legit symbol
              ...
              this._createAndRecordToken(tokenSet, line, lineIdx, referenceOffset, referencePart.length, 'variable', ['illegalUse'], referencePart);
      ...
      if (nextString.length > 0) {
        ...
        if (bStringContainsBackTic) {
          ...
          while (backTicOffset !== -1) {
            ...
            this._createAndRecordToken(tokenSet, line, lineIdx, lineStrOffset, currStr.length, 'string', ['quoted', 'single']);
            ...
            if (closeParenOffset !== -1) {
              ...
              if (backTicOffset !== -1) {
                ...
                this._createAndRecordToken(tokenSet, line, lineIdx, lineStrOffset, rightStr.length, 'string', ['quoted', 'single']);
          ...
          this._createAndRecordToken(tokenSet, line, lineIdx, strOffset, nextString.length, 'string', ['quoted', 'single']);
        }
      ...
      if (nextString.length > 0) {
        ...
        if (bStringContainsBackTic) {
          ...
          while ((backTicOffset = nextString.indexOf(chrBackTic, searchOffset)) !== -1) {
            ...
            // record the left edge string
            this._createAndRecordToken(tokenSet, line, lineIdx, lineStrOffset, leftStr.length, 'string', ['quoted', 'double']);
            ...
            if (closeParenOffset !== -1) {
              ...
              // record the right edge string
              this._createAndRecordToken(tokenSet, line, lineIdx, rightStrOffset, rightStr.length, 'string', ['quoted', 'double']);
          ...
          this._createAndRecordToken(tokenSet, line, lineIdx, strOffset, nextString.length, 'string', ['quoted', 'double']);
        }
  ...
  private _createToken(lineIdx: number, startChar: number, length: number, tokenType: string, modifiers: string[] = []): IParsedToken {
    return {
      line: lineIdx,
  ...
  private _createAndRecordToken(
    ...
    this._recordToken(tokenSet, line, this._createToken(lineIdx, startChar, length, tokenType, modifiers), tokenName);
  }
  ...
  private _getGlobalTokenIfExists(
    symbolName: string,
    logCategory?: 'TokenSet' | 'State' | 'SPIN' | 'PreProc' | 'CON' | 'VAR' | 'DAT' | 'OBJ' | 'PASM' | 'DEBUG'
  ): RememberedToken | undefined {
  ...
  private _getLocalTokenIfExists(
    symbolName: string,
    lineNbr: number,
    logCategory?: 'TokenSet' | 'State' | 'SPIN' | 'PreProc' | 'CON' | 'VAR' | 'DAT' | 'OBJ' | 'PASM' | 'DEBUG'
  ): RememberedToken | undefined {
  ...
  private _conditionalLog(
    category: 'TokenSet' | 'State' | 'SPIN' | 'PreProc' | 'CON' | 'VAR' | 'DAT' | 'OBJ' | 'PASM' | 'DEBUG',
    message: string
  ): void {
    ...
    if (this.parseUtils.requestedSpinVersion(50)) {
      ...
      for (let chrIdx = startingOffset + 2; chrIdx < workString.length; chrIdx++) {
        ...
        if (bInEscape) {
          ...
          if (bValidSequence) {
            this._createAndRecordToken(tokenSet, line, lineIdx, chrIdx - 1, sequenceLength, 'operator', ['builtin']);
          }
