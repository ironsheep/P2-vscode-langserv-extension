...
export class Spin2ParseUtils {
  ...
  public removeDebugSingleQuotedStrings(line: string, showDebug: boolean = true): string {
    ...
    let ticEscapeCloseParenOffset: number = -1;
    ...
    let badParse: boolean = false;
    // if we have any quoted strings
    if (quoteStartOffset != -1) {
      // SKIP past ticEscapes to locate last closing paren
      while (nextTicEscape != -1) {
        lastCloseParenOffset = trimmedLine.indexOf(chrCloseParen, nextTicEscape + 1);
        if (lastCloseParenOffset == -1) {
          badParse = true;
          break; // error abort loop!
        }
        // if we have `( followed by ) then skip this close, look for next
        nextTicEscape = this._nextTicEscape(trimmedLine, lastCloseParenOffset + 1);
        // do we have another?
        if (nextTicEscape == -1) {
          // no, now locate our last closing paren
          lastCloseParenOffset = trimmedLine.indexOf(chrCloseParen, lastCloseParenOffset + 1);
          if (lastCloseParenOffset == -1) {
            badParse = true;
            break; // error abort loop! (this one shouldn't be needed as we are already ending the loop, but you know what I intend!)
          }
        }
      }
  ...
  private _nextTicEscape(line: string, startingOffset: number): number {
    ...
    let nextDecimal: number = line.indexOf("`(", startingOffset);
    let nextHex: number = line.indexOf("`$(", startingOffset);
    let nextBinary: number = line.indexOf("`%(", startingOffset);
    let nextCharacter: number = line.indexOf("`#(", startingOffset);
    if (nextDecimal != -1) {
  ...
  private _removeInlineComments(startingOffset: number, line: string): string {
    ...
    //do {
    const doubleBraceBeginOffset: number = cleanedLine.indexOf("{{", startingOffset);
    if (doubleBraceBeginOffset != -1) {
      const doubleBraceEndOffset: number = cleanedLine.indexOf("}}", doubleBraceBeginOffset);
    ...
    //} while (cleanedLine.indexOf("{{", startingOffset) != -1);

    //   REPLACE {...} when found, all occurrences
    //do {
    const singleBraceBeginOffset: number = cleanedLine.indexOf("{", startingOffset);
    if (singleBraceBeginOffset != -1) {
      const singleBraceEndOffset: number = cleanedLine.indexOf("}", singleBraceBeginOffset);
      if (singleBraceEndOffset != -1) {
    ...
    const doubleBraceEndOffset: number = cleanedLine.indexOf("}}", startingOffset);
    if (doubleBraceEndOffset != -1) {
    ...
    //   REPLACE ^...} when NO { before it on start of line
    const singleBraceEndOffset: number = cleanedLine.indexOf("}", startingOffset);
    if (singleBraceEndOffset != -1) {
  ...
  public getLineWithoutInlineComments(line: string): string {
    ...
    //   REPLACE {{...}} when found, all occurrences
    //do {
    const doubleBraceBeginOffset: number = cleanedLine.indexOf("{{");
    if (doubleBraceBeginOffset != -1) {
      const doubleBraceEndOffset: number = cleanedLine.indexOf("}}", doubleBraceBeginOffset);
      if (doubleBraceEndOffset != -1) {
        const inLineComment: string = cleanedLine.substring(doubleBraceBeginOffset, doubleBraceEndOffset + 2);
    ...
    //   REPLACE {...} when found, all occurrences
    //do {
    const singleBraceBeginOffset: number = cleanedLine.indexOf("{");
    if (singleBraceBeginOffset != -1) {
      const singleBraceEndOffset: number = cleanedLine.indexOf("}", singleBraceBeginOffset);
      if (singleBraceEndOffset != -1) {
        const inLineComment: string = cleanedLine.substring(singleBraceBeginOffset, singleBraceEndOffset + 1);
